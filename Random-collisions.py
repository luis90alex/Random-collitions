...
class Particle:
    ...
    def draw(self, ax):
        ...
        circle = Circle(xy=self.r, radius=self.radius,color="red")
        ax.add_patch(circle)
        return circle
...
class Simulation:
    ...
    def handle_collisions(self):
        ...
        for i,j in pairs:
            if self.particles[i].overlaps(self.particles[j]):
                ...
                number=np.random.normal(mu,0.25)
                
                if mu-sigma< number < mu+sigma :
                    self.circles[i].set_color("black")
                    self.circles[j].set_color("black")
    ...
    def do_animation(self, save=False, interval=1, filename='collision.mp4'):
        ...
        anim = animation.FuncAnimation(self.fig, self.animate,
                init_func=self.init, frames=2000, interval=interval, blit=True)
        self.save_or_show_animation(anim, save, filename)
        

if __name__ == '__main__':
    nparticles = 50
    radii = 0.02#np.random.random(nparticles)*0.03+0.02
